...
class MapHouse extends React.Component {
  ...
  componentDidMount() {
    ...
    let showInfo = () => {
        ...
        client.query({query: GET_MAP_HOUSE, variables:
                {"lng":cp.lng,"lat":cp.lat,"zoom":zoom}}).then(result =>{
    ...
    client.query({query: GET_MAP_HOUSE, variables:{"lng":defaultX,"lat":defaultY,"zoom":defaultZoom}}).then(result =>{
       //数据加载完成
      let xys = result.data.MapHouseData.list;
      showMapMarker(xys, map);
    });
